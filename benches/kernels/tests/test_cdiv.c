...
static int run_one(const ops_vtable_t* ref, const ops_vtable_t* hw,
                   uint64_t a, uint64_t b, int verbose)
{
  uint64_t r = ref->pdiv8(a, b);
  uint64_t h = hw->pdiv8(a, b);
  if (r != h) {
    if (verbose) {
      printf("PDIV8 mismatch!\n");
...
int main(void)
{
  ...
  { // div-by-zero -> 0
    int8_t av[8] = { 10, -10,  50, -50, 127, -128, 1, -1 };
    int8_t bv[8] = {  0,   0,   0,   0,   0,    0, 0,  0 };
    errors += run_one(ref, hw, pack_i8(av), pack_i8(bv), 1);
  }
  ...
  for (int i = 0; i < N; ++i) {
    ...
    errors += run_one(ref, hw, a, b, 0);
    if (errors > 50) break;
  }

  if (errors == 0) {
    printf("Correctness: PDIV8 PASSED (%d random tests)\n", N);
    return 0;
  }
  printf("Correctness: PDIV8 FAILED (%d errors)\n", errors);
  return 1;
}
